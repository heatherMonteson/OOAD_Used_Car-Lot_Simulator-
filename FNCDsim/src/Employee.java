...
public abstract class Employee implements Utility, Name{
    ...
    public Employee getNewSalesPerson(Employee oldSales){
        //we are passing in a current salesperson because we do not want that one

        
        Random rand = new Random();
        //same size so does not matter
        int randomIndex = rand.nextInt(AskName.salesPeopleNorth.size());

        //NORTH:
        if(FNCDsim.currentDealership == Enums.FNCD_location.FNCD_North){
            Employee NorthrandomSalesPerson = AskName.salesPeopleNorth.get(randomIndex);
            // Check if the random salesperson is the same as the parameter staffPerson
            // If it is, select another random salesperson until we get one that is different
            while (NorthrandomSalesPerson.equals(oldSales)) {
                randomIndex = rand.nextInt(AskName.salesPeopleNorth.size());
                NorthrandomSalesPerson = AskName.salesPeopleNorth.get(randomIndex);
            }
            return NorthrandomSalesPerson;

        }else{
             //SOUTH:
            Employee SouthrandomSalesPerson = AskName.salesPeopleSouth.get(randomIndex);
            // Check if the random salesperson is the same as the parameter staffPerson
            // If it is, select another random salesperson until we get one that is different
            while (SouthrandomSalesPerson.equals(oldSales)) {
                randomIndex = rand.nextInt(AskName.salesPeopleSouth.size());
                SouthrandomSalesPerson = AskName.salesPeopleSouth.get(randomIndex);
            }
            return SouthrandomSalesPerson;
        }
     

    }

    //added for command line interface
    ...
    public double getTotalBonusPay(){return totalBonusPay;}
    ...
    //use random name generator interface to select a name using first names and a last name initial
    ...
    public void payBonus(double bonus){
        incomeToDate=Utility.format(incomeToDate+bonus);
        totalBonusPay=Utility.format(totalBonusPay+bonus);
    }
...
class Intern extends Employee {
    ...
    private void setCleaningBehavior(){
        ...
        if (randomNum == 1)
            cleanBehavior = new ChemicalClean();
        else if (randomNum == 2)
            cleanBehavior = new Detailed();
        else
            cleanBehavior = new ElbowGrease();
    }
